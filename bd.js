...
client.on('ready', async function() {
	...
	print("[알림] 백업 용도로 채널의 메시지를 CSV로 백업해드립니다...\r\n");
	...
	rl.question("\r\n아무거나 입력하시고 엔터: ", (answer) => {
		...
		rl2.question("제외할 메시지 키워드(없으면 빈칸): ", (answer2) => {
			...
			print("\r\n삭제를 시작합니다. 취소하려면 3초 이내에 <Ctrl+C>을 누르십시오.\r\n");
						...
						if(i <= 12345678) {
							setTimeout(async function() { // 3초마다 100개씩 가져오기. 한 번에 해 버리면 일시적 차단이 되므로 하지말것.
								...
								for(var msg of msgs) {
									...
									if(cm.author.username == 'gdl-888') {
										setTimeout(() => {
											cm.delete(300).then(() => {
												print(`[[${cm.content.slice(0, 10)} 삭제 완료.]]`);
											}).catch(e => {
												print(`[[${cm.content.slice(0, 10)} 삭제 실패!]]`);
											});
								...
								if(Number(sid) > Number(lid) || save) { // 모든 메시지를 가져왔을 때 화일로 저장하기.
									...
									print(`선택한 채널의 삭제 전 원본 메시지는 ${fn}에 저장되었읍니다.`);
									print('삭제가 완전히 이루어지지 않았을 확률이 크므로 창을 닫지 마십시오.');
